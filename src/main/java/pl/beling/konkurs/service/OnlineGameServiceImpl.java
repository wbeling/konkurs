...
public class OnlineGameServiceImpl implements OnlineGameService {
    ...
    public List<List<ClanDto>> calculate(PlayersDto players) {
        ...
        clans.forEach(clan -> {
            // flag to mark if we need to create a new group
            ...
            for (int i = 0; i < returnList.size(); i++) {
                ...
                if(currentSizeOfGroup < maxGroupSize) { // if the group is not full already
                    ...
                    if (newSizeOfGroup <= maxGroupSize) { // check if we won't exceed the max size by adding clan,
                        returnList.get(i).add(clan); // we add this clan to this group with index 'i'
                        groupSizes[i] = newSizeOfGroup; // update current size
                        newGroupIsNeeded = false; // we found the group, we won't have to create a new one
                        break;
                    }
                }
            }
